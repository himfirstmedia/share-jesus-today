...
class ApiService {
  ...
  private SINGLE_UPLOAD_MAX_SIZE_BYTES = 20 * 1024 * 1024; // 20 MB
  ...
  private async makeRequest<T>(
    ...
    try {
      ...
      if (config.body && typeof config.body === 'string') {
        console.log('API Request - Body (JSON):', config.body);
      }
      ...
      if (!response.ok) {
        ...
        try {
          ...
          console.error('API Error - Failed to parse error JSON:', jsonError);
        }
  ...
  async uploadVideo(
    ...
    try {
      ...
      if (!fileInfo.exists) {
        throw new Error(`Video file does not exist: ${videoFile.uri}`);
      }

      if (fileInfo.size > this.SINGLE_UPLOAD_MAX_SIZE_BYTES) {
        console.log(`VideoAPI: File size (${fileInfo.size} bytes) exceeds single upload limit. Initiating chunked upload.`);
        return this.uploadVideoInChunks(videoFile, metadata, progressCallback);
      }

      console.log(`VideoAPI: File size (${fileInfo.size} bytes) within single upload limit. Proceeding with direct upload.`);
      ...
      if (!response.ok) {
        ...
        try {
          ...
          errorMessage = `HTTP ${response.status}: ${response.statusText}`;
          console.error('Video Upload Error - Failed to parse error JSON:', jsonError);
        }
        console.error('Video Upload Error - Response details:', errorDetails);
      ...
      const data = await response.json();
      console.log('Video Upload Success - Response data:', data);
  ...
  // New method for chunked video upload
  async uploadVideoInChunks(
    ...
    try {
      const fileInfo = await FileSystem.getInfoAsync(videoFile.uri);
      if (!fileInfo.exists) {
        throw new Error('File does not exist for chunked upload');
      }

      const fileSize = fileInfo.size;
      const CHUNK_SIZE = 5 * 1024 * 1024; // 5 MB chunks
      const totalChunks = Math.ceil(fileSize / CHUNK_SIZE);
      const uploadId = `${videoFile.name}-${Date.now()}`; // Unique ID for this upload

      console.log(`Starting chunked upload for ${videoFile.name}, size: ${fileSize} bytes, chunks: ${totalChunks}`);

      for (let i = 0; i < totalChunks; i++) {
        const offset = i * CHUNK_SIZE;
        const chunk = await FileSystem.readAsStringAsync(videoFile.uri, {
        ...
        const formData = new FormData();
        formData.append('file', {
          uri: `data:${videoFile.type};base64,${chunk}`,
          name: videoFile.name,
          type: videoFile.type,
        } as any);
        ...
        const response = await fetch(`${this.baseURL}/video/upload/chunk`, { // New endpoint for chunks
          method: 'POST',
          headers: headers,
          body: formData,
        });
        ...
        if (!response.ok) {
          ...
          throw new Error(`Chunk upload failed for chunk ${i + 1}: HTTP ${response.status} - ${errorData}`);
        }

        progressCallback(((i + 1) / totalChunks) * 0.9); // 90% for chunk uploads

        console.log(`Chunk ${i + 1}/${totalChunks} uploaded successfully.`);
      }

      // Finalize upload request
      console.log('All chunks uploaded, sending finalize request.');
      const finalizeResponse = await this.makeRequest<any>('/video/upload/finalize', { // New endpoint to finalize
        method: 'POST',
        body: JSON.stringify({
      ...
      if (!finalizeResponse.success) {
        throw new Error(`Upload finalization failed: ${finalizeResponse.error}`);
      }

      progressCallback(1.0); // 100% for completion

      console.log('Video Upload Success - Chunked upload completed.');
      return {
        success: true,
      ...
      console.error('Chunked video upload failed:', error);
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Chunked upload error occurred',
      };
    }
  }
